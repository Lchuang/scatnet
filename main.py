...
with tf.compat.v1.Session(graph=g, config=config) as sess:
    ...
    with g.as_default():
        ...
        sx_w = tf.compat.v1.placeholder(tf.float32, (n_features, n_pca))
        sx_bar = tf.compat.v1.placeholder(tf.float32, shape=(1, n_features))
        ...
        print(n_clusters)
        mu = tf.compat.v1.placeholder(tf.float32, shape=(n_clusters, n_pca))
        cov = tf.compat.v1.placeholder(tf.float32, shape=(n_clusters, n_pca, n_pca))
        tau = tf.compat.v1.placeholder(tf.float32, shape=(n_clusters,))

        # Calculate Gaussian mixture loss
        #loss, cat = sn.models.gmm(mu, cov, tau, sx_proj,n_clusters, **args['gmm'])
        loss, cat = sn.models.gmm(mu, cov, tau, sx_proj, n_clusters, **args['gmm'])
